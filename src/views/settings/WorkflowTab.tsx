...
export const WorkflowTab: React.FC<WorkflowTabProps> = ({ form, setForm }) => {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── Driveway Letter Settings ──────────────────────────────────────────── */}
      <div className="border-t border-slate-100 dark:border-slate-700 pt-6">
        <h2 className="text-base font-bold text-slate-800 dark:text-slate-200 mb-1">Driveway Letter Rules</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
          Controls Metro review timers and lead time alerts on driveway impact letters.
        </p>
        <div className="grid grid-cols-2 gap-6 max-w-lg">
          <div>
            <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1 block">
              Metro Review SLA (days)
            </label>
            <input
              type="number" min={1}
              className="w-28 border border-red-300 bg-red-50 rounded-lg px-3 py-2 text-sm font-semibold text-red-800 focus:outline-none focus:ring-2 focus:ring-red-400"
              value={form.driveway_metroSLADays ?? 5}
              onChange={e => setForm(p => ({ ...p, driveway_metroSLADays: parseInt(e.target.value) || 5 }))}
            />
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Letters in Metro's court turn red after this many days.</p>
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1 block">
              Metro Review Warning (days)
            </label>
            <input
              type="number" min={1}
              className="w-28 border border-amber-300 bg-amber-50 rounded-lg px-3 py-2 text-sm font-semibold text-amber-800 focus:outline-none focus:ring-2 focus:ring-amber-400"
              value={form.driveway_metroWarnDays ?? 3}
              onChange={e => setForm(p => ({ ...p, driveway_metroWarnDays: parseInt(e.target.value) || 3 }))}
            />
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Letters in Metro's court turn amber after this many days.</p>
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1 block">
            ...
            <input
              type="number" min={1}
              className="w-28 border border-slate-300 bg-slate-50 rounded-lg px-3 py-2 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-400"
              value={form.driveway_leadTimeDays ?? 10}
              onChange={e => setForm(p => ({ ...p, driveway_leadTimeDays: parseInt(e.target.value) || 10 }))}
            />
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Alert when driveway impact is flagged without this many days of runway.</p>
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1 block">
              Date-Shift Reissue Threshold (days)
            </label>
            <input
              type="number" min={1}
              className="w-28 border border-slate-300 bg-slate-50 rounded-lg px-3 py-2 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-400"
              value={form.driveway_reissueDays ?? 5}
              onChange={e => setForm(p => ({ ...p, driveway_reissueDays: parseInt(e.target.value) || 5 }))}
            />
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Flag for CR decision when plan dates shift more than this after a letter is sent.</p>
          </div>
        </div>
      </div>
    </div>
