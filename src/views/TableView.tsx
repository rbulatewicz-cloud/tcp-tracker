  ...
  return (
    <>
      ...
      <div style={{display:"grid",gridTemplateColumns:`repeat(${STAGES.length},1fr)`,gap:8,marginBottom:16}}>
        {STAGES.map(s=>{const c=plans.filter(p=>p.stage===s.key).length;const act=filter.stage===s.key;return(
          <div key={s.key} onClick={()=>setFilter(f=>({...f,stage:act?"all":s.key}))} style={{background:"#fff",border:`1.5px solid ${act?s.color:"#E2E8F0"}`,borderRadius:10,padding:"12px 10px",cursor:"pointer",textAlign:"center",transition:"all 0.2s",boxShadow:act?`0 0 0 3px ${s.color}22`:"none"}}>
            <div style={{fontSize:24,fontWeight:800,color:s.color,fontFamily:monoFont,lineHeight:1}}>{c}</div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
        ...
        {([{key:"type",options:PLAN_TYPES,label:"Type"},{key:"lead",options:LEADS,label:"Lead"},{key:"priority",options:PRIORITIES,label:"Priority"}] as {key: keyof FilterState; options: string[]; label: string}[]).map((f)=>(
          <select key={f.key} value={filter[f.key] || "all"} onChange={e=>setFilter(pr=>({...pr,[f.key]:e.target.value}))} style={{...inp,width:"auto",padding:"7px 12px",fontSize:11,cursor:"pointer",background:"#fff"}}><option value="all">All {f.label}s</option>{f.options.map((o: string)=><option key={o} value={o}>{o}</option>)}</select>
        ))}
        ...
        {canEditPlan && selectedPlanIds.length > 0 && (
          <div style={{display:"flex", alignItems:"center", gap:10, background:"#0F172A", padding:"4px 12px", borderRadius:8, color:"#fff", fontSize:11, fontWeight:600}}>
            ...
            <select 
              onChange={(e) => bulkUpdate({lead: e.target.value}, null)}
              disabled={loading.bulk || (currentUser?.role !== UserRole.MOT && currentUser?.role !== UserRole.ADMIN)}
              style={{background:"transparent", color:"#fff", border:"none", fontSize:11, fontWeight:600, outline:"none", cursor:(loading.bulk || (currentUser?.role !== UserRole.MOT && currentUser?.role !== UserRole.ADMIN)) ? "not-allowed" : "pointer"}}
              defaultValue=""
            >
            ...
            <select 
              onChange={(e) => bulkUpdate({priority: e.target.value}, null)}
              disabled={loading.bulk}
              style={{background:"transparent", color:"#fff", border:"none", fontSize:11, fontWeight:600, outline:"none", cursor:loading.bulk ? "not-allowed" : "pointer"}}
              defaultValue=""
            >
            ...
            <select 
              onChange={(e) => setStatusDateModal({status: e.target.value, date: td})}
              disabled={loading.bulk}
              style={{background:"transparent", color:"#fff", border:"none", fontSize:11, fontWeight:600, outline:"none", cursor:loading.bulk ? "not-allowed" : "pointer"}}
              defaultValue=""
            >
              <option value="" disabled style={{color:"#000"}}>Set Status...</option>
              {STAGES.map(s => <option key={s.key} value={s.key} style={{color:"#000"}}>{s.label}</option>)}
            </select>
      ...
      <div style={{display: "flex", flexWrap: "wrap", gap: 16, marginBottom: 12, padding: "10px 16px", background: "#fff", borderRadius: 12, border: "1px solid #E2E8F0", fontSize: 11}}>
        <div style={{display: "flex", alignItems: "center", gap: 6}}>
          ...
          <div style={{width: 12, height: 12, borderRadius: 3, background: "#FFFBEB", border: "1px solid #FDE68A"}}></div>
        ...
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
          <thead><tr style={{background:"#F8FAFC",borderBottom:"2px solid #E2E8F0"}}>
            {canEditPlan && (
              <th style={{padding:"10px 8px", width:30, textAlign:"center"}}>
                <input 
                  type="checkbox" 
                  checked={selectedPlanIds.length === filtered.length && filtered.length > 0} 
          ...
          <tbody>
            {sortedData.map((plan,idx)=>{
              ...
              const wd: any=plan.submitDate&&!COMPLETED_STAGES.includes(plan.stage)?daysBetween(plan.submitDate,td):plan.submitDate&&plan.approvedDate?daysBetween(plan.submitDate,plan.approvedDate):null;
              const isSelected = selectedPlanIds.includes(plan.id);
              return(
                <tr key={plan.id} style={{borderBottom:"1px solid #F1F5F9",cursor:"pointer",background:isSelected?"#F0F9FF":isRisk?"#FFFBEB":idx%2===0?"#fff":"#FAFBFC",transition:"background 0.1s"}} onMouseEnter={e=>e.currentTarget.style.background="#F0F9FF"} onMouseLeave={e=>e.currentTarget.style.background=isSelected?"#F0F9FF":isRisk?"#FFFBEB":idx%2===0?"#fff":"#FAFBFC"}>
                  {canEditPlan && (
                  ...
                  {mainCols.map(col => {
                    switch(col.id) {
                      case "id": return <td key={col.id} onClick={()=>setSelectedPlan(plan)} style={{padding:"10px 8px",fontFamily:monoFont,fontWeight:700,color:"#64748B",fontSize:11}}>{plan.id}</td>;
                      case "rev": return <td key={col.id} onClick={()=>setSelectedPlan(plan)} style={{padding:"10px 8px",fontFamily:monoFont,fontWeight:700,color:"#64748B",fontSize:11}}>{plan.rev || 0}</td>;
                      case "loc": return <td key={col.id} onClick={()=>setSelectedPlan(plan)} style={{padding:"10px 8px",fontFamily:monoFont,fontWeight:700,color:"#D97706",fontSize:12}}>{plan.loc||"TBD"}</td>;
                      case "type": return <td key={col.id} onClick={()=>setSelectedPlan(plan)} style={{padding:"10px 8px",color:"#64748B",fontSize:11}}>{plan.type}</td>;
                      case "scope": return <td key={col.id} onClick={()=>setSelectedPlan(plan)} style={{padding:"10px 8px",fontSize:11}}>{plan.scope}</td>;
                      case "segment": return <td key={col.id} onClick={()=>setSelectedPlan(plan)} style={{padding:"10px 8px",fontFamily:monoFont,color:"#94A3B8",fontSize:11}}>{plan.segment}</td>;
                      case "location": return <td key={col.id} onClick={()=>setSelectedPlan(plan)} style={{padding:"10px 8px",fontSize:12,fontWeight:500}}>{plan.street1}{plan.street2?` / ${plan.street2}`:""}</td>;
                      case "lead": return <td key={col.id} onClick={()=>setSelectedPlan(plan)} style={{padding:"10px 8px",fontSize:11}}>{plan.lead}</td>;
                      case "priority": return <td key={col.id} onClick={()=>setSelectedPlan(plan)} style={{padding:"10px 8px"}}><span style={{padding:"3px 8px",borderRadius:4,fontSize:9,fontWeight:700,letterSpacing:0.3,background:plan.priority==="Critical"?"#FEE2E2":plan.priority==="High"?"#FEF3C7":plan.priority==="Medium"?"#DBEAFE":"#F1F5F9",color:plan.priority==="Critical"?"#DC2626":plan.priority==="High"?"#D97706":plan.priority==="Medium"?"#2563EB":"#64748B"}}>{plan.priority.toUpperCase()}</span></td>;
                      case "status": return <td key={col.id} onClick={()=>setSelectedPlan(plan)} style={{padding:"10px 8px"}}><span style={{padding:"3px 8px",borderRadius:4,fontSize:9,fontWeight:700,background:`${stage.color}18`,color:stage.color,letterSpacing:0.3}}>{stage.label.toUpperCase()}</span></td>;
                      case "submittedToDOT": return <td key={col.id} onClick={()=>setSelectedPlan(plan)} style={{padding:"10px 8px",fontFamily:monoFont,fontSize:10,color:"#94A3B8"}}>{plan.submitDate||"—"}</td>;
                      case "requested": return <td key={col.id} onClick={()=>setSelectedPlan(plan)} style={{padding:"10px 8px",fontFamily:monoFont,fontSize:10,color:"#94A3B8"}}>{plan.dateRequested||plan.requestDate||"—"}</td>;
                      case "needBy": return <td key={col.id} onClick={()=>setSelectedPlan(plan)} style={{padding:"10px 8px",fontFamily:monoFont,fontSize:10,color:dl<=0?"#DC2626":dl<=7?"#D97706":"#64748B"}}>{plan.needByDate}</td>;
                      case "wait": return <td key={col.id} onClick={()=>setSelectedPlan(plan)} style={{padding:"10px 8px",fontFamily:monoFont,fontWeight:700,fontSize:12,textAlign:"center",color:COMPLETED_STAGES.includes(plan.stage)?"#10B981":wd>20?"#DC2626":wd>10?"#D97706":"#64748B"}}>{COMPLETED_STAGES.includes(plan.stage)?"✓":wd!==null?`${wd}d`:"—"}</td>;
                      default: return null;
                    }
                  })}
                </tr>
            ...
            {filtered.length===0&&<tr><td colSpan={canEditPlan ? 13 : 12} style={{padding:40,textAlign:"center",color:"#94A3B8"}}>No plans match filters</td></tr>}
          </tbody>
        </table>
      </div>
    </>
