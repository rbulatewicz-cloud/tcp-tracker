      ...
      <div style={{padding:"20px 28px"}}>
        ...
        {view==="log" && (
          <GlobalActivityLogView
            ...
            searchQuery={searchQuery}
            setSearchQuery={setSearchQuery}
            setClearLogConfirm={setClearLogConfirm}
            logCols={logCols}
            plans={plans}
            setSelectedPlan={setSelectedPlan}
            setView={setView}
      ...
      {clearLogConfirm.isOpen && (
        <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.6)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:20}}>
          <div style={{background:"#fff",borderRadius:16,padding:32,width:"100%",maxWidth:400,boxShadow:"0 25px 50px rgba(0,0,0,0.15)", textAlign:"center"}}>
            ...
            <div style={{display:"flex",gap:12}}>
              <button onClick={() => setClearLogConfirm({isOpen: false, type: 'plan', planId: null})} style={{flex:1,background:"#F1F5F9",color:"#475569",border:"none",padding:"10px",borderRadius:8,fontWeight:700,cursor:"pointer"}}>Cancel</button>
              <button onClick={() => handleClearLog(clearLogConfirm)} style={{flex:1,background:"#EF4444",color:"#fff",border:"none",padding:"10px",borderRadius:8,fontWeight:700,cursor:"pointer"}}>
                {loading.bulk ? "Clearing..." : "Yes, Clear Log"}
              </button>
            </div>
          </div>
        </div>
                ...
                {selectedPlan && (
                  <>
                    ...
                    {(currentUser?.role === UserRole.MOT || currentUser?.role === UserRole.ADMIN) && (
                      <button
                        onClick={() => setClearLogConfirm({ isOpen: true, type: 'plan', planId: selectedPlan.id })}
                        style={{ background: "#FEF2F2", color: "#EF4444", border: "1px solid #FECACA", padding: "4px 8px", borderRadius: 4, fontSize: 9, fontWeight: 700, cursor: "pointer" }}
                      >
                        Clear Log
                      </button>
                    )}
